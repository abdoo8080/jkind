...
/**
 * This class represents a Kind 2 function. Kind 2 supports the {@code function} keyword which is
 * used just like the {@code node} one but has slightly different semantics. Like the name suggests,
 * the output(s) of a function should be a <i>non-temporal</i> combination of its inputs. That is, a
...
public class Kind2Function extends Ast {
	 ...
	 * @param equations    equations relating inputs and locals to other locals and outputs
	...
	public Kind2Function(Location location, String id, List<VarDecl> inputs, List<VarDecl> outputs,
			ContractBody contractBody, List<VarDecl> locals, List<Equation> equations,
			List<Expr> assertions, List<String> properties) {
		super(location);
		Assert.isNotNull(id);
		this.id = id;
	 ...
	 * @param properties   properties to check on this function
	 */
	public Kind2Function(String id, List<VarDecl> inputs, ContractBody contractBody,
			List<VarDecl> outputs, List<VarDecl> locals, List<Equation> equations,
			List<Expr> assertions, List<String> properties) {
		this(Location.NULL, id, inputs, outputs, contractBody, locals, equations, assertions,
				properties);
	}
